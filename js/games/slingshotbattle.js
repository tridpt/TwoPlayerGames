/* Slingshot Battle - keo tha ban da/phep qua chuong ngai. */
(function () {
  const W = 820;
  const H = 470;
  const FLOOR = 398;
  ...
  function create(ctx) {
    ...
    let explosion = null;
    let raf = null;
    let last = "Kéo để ngắm, thả để bắn.";
    ...
    function makePlayer(idx, x, color, facing) {
      return {
        ...
        color,
        facing,
        aim: { dx: facing * 100, dy: -82 },
      };
    }

    function makeObstacles(count) {
      const out = [];
      function add(x, y, w, h, hp) {
        out.push({ x, y, w, h, hp, maxHp: hp, active: true });
      }
    ...
    function startProjectile(pl, spellId) {
      ...
      projectile = {
        ...
        x: a.x,
        y: a.y,
        vx: pl.aim.dx * POWER_SCALE,
        vy: pl.aim.dy * POWER_SCALE,
        r: spell.radius,
    ...
    function hitBounds(prev) {
      ...
      if (!p) return true;
      if (p.y >= FLOOR - p.r) {
        explodeAt(p.x, FLOOR - p.r);
        return true;
      }
      if (p.x < 22 + p.r) {
        p.x = 22 + p.r;
        return bounceOrExplode("x");
      }
      if (p.x > W - 22 - p.r) {
        p.x = W - 22 - p.r;
        return bounceOrExplode("x");
    ...
    function hitPlayer() {
      ...
      for (const pl of players) {
        ...
        if (dist(p.x, p.y, pl.x, pl.y) <= HERO_R + p.r) {
          explodeAt(p.x, p.y);
          return true;
        }
      }
    ...
    function explodeAt(x, y) {
      ...
      players.forEach((pl) => {
        ...
        const reach = spell.blast + HERO_R;
        if (d >= reach) return;
        const dmg = Math.max(1, Math.round(spell.dmg * (1 - d / reach)));
        pl.hp = Math.max(0, pl.hp - dmg);
        messages.push(`gây ${dmg} sát thương P${pl.idx + 1}`);
      });
      obstacles.forEach((ob) => {
        if (!ob.active) return;
        const d = rectDistance(x, y, ob);
        if (d >= spell.blast) return;
        ob.hp -= Math.round(spell.dmg * 0.7 * (1 - d / spell.blast) + 8);
        if (ob.hp <= 0) ob.active = false;
      });
      last = messages.length
        ? `Người chơi ${owner + 1} ${messages.join(", ")}.`
        : `Cú bắn của Người chơi ${owner + 1} nổ hụt.`;
      ctx.sound(messages.length ? "capture" : "miss");
      renderHud();
    }

    function finishExplosion() {
      if (!explosion || explosion.resolved || explosion.r < explosion.max + STOP_EXPLOSION) return;
      ...
      if (dead0 || dead1) {
        ...
        ctx.setTurn(-1);
        if (dead0 && dead1) {
          ctx.setStatus("🤝 Hai bên cùng gục - hòa!");
        } else {
      ...
      selectedSpell = players[turn].mana >= SPELLS[selectedSpell]?.cost ? selectedSpell : "stone";
      ctx.setTurn(turn);
      renderHud();
      syncControls();
      updateStatus();
    ...
    function renderHud() {
      hud.innerHTML = `
        ...
        <div class="sling-mid">
          ...
          <span>${windText()} · Chướng ngại còn ${obstacles.filter((o) => o.active).length}</span>
          <small>${last}</small>
        </div>
        ${playerHud(1)}
      `;
    ...
    function updateStatus() {
      if (over) return;
      if (busy) {
        ctx.setStatus("Đạn đang bay, chờ nổ xong rồi đổi lượt.");
      } else if (ctx.isOnline && turn !== ctx.mySeat) {
        ctx.setStatus(`Đối thủ đang kéo ngắm. ${last}`);
      } else {
        ctx.setStatus(`Người chơi ${turn + 1}: kéo từ nhân vật để ngắm, chọn phép rồi thả hoặc bấm Bắn.`);
      }
    }
    ...
    function drawExplosion() {
      if (!explosion) return;
      g.save();
      const alpha = Math.max(0, 1 - explosion.r / (explosion.max + STOP_EXPLOSION));
      g.globalAlpha = alpha;
      g.fillStyle = explosion.color;
      g.beginPath();
      ...
      g.lineWidth = 3;
      g.beginPath();
      g.arc(explosion.x, explosion.y, Math.max(3, explosion.r * 0.68), 0, Math.PI * 2);
      g.stroke();
      g.restore();
    }
    ...
    function loop() {
      ...
      if (explosion) {
        explosion.r += 4.5;
        finishExplosion();
        if (explosion && explosion.resolved) explosion = null;
      }
      draw();
  ...
  window.GameRegistry.register({
    ...
    emoji: "🏹",
    description: "Kéo thả để bắn đá hoặc phép qua chướng ngại, tính gió, bật tường và nổ gây sát thương.",
    onlineReady: true,
    options: [
      {
        id: "hp",
    ...
    howTo: [
      "Hai nhân vật đứng hai bên sân. Đến lượt mình, kéo từ nhân vật ra phía sau để căng dây, thả ra để bắn.",
      "Có thể bấm Bắn để dùng hướng kéo hiện tại. Đường chấm trên sân cho biết hướng bay dự kiến.",
      "Đá miễn phí và có thể nảy một lần. Cầu lửa tốn 2 mana, nổ lớn nhưng không nảy.",
      "Bật tường tốn 1 mana và nảy nhiều lần, hợp để bắn vòng qua chướng ngại. Bùa nhẹ tốn 2 mana, bay xa hơn vì ít chịu trọng lực.",
      "Gió đổi mỗi lượt. Chướng ngại có máu, bị va chạm hoặc nổ gần sẽ vỡ dần.",
      "Vụ nổ càng gần nhân vật càng gây nhiều sát thương. Hạ đối thủ trước để thắng.",
      "Chơi online: mỗi lượt chỉ gửi vector kéo và loại đạn, hai máy tự mô phỏng cùng kết quả.",
    ],
    create,
  });
})();
